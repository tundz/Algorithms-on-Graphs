...
public class DistWithCoords {
    private static class Impl {
        ...
        //long[] reverseEucldist;
        
        //forward potential for A star, reverse potential is simply Pr(u) = - Pf(u)
         //long[] forwardPotential;  
      
         //The new EdgeWeights recomputed for Astart search
         ArrayList<Long>[] newForwardCost;
         //ArrayList<Long>[] newReverseCost;
        ...
        Impl(int n) {
            ...
            frwdistance = new long[n]; //revdistance = new long[n];
            forwardEucldist = new long[n]; //reverseEucldist = new long[n];
            //forwardPotential = new long[n];
            frwProcessed = new boolean[n]; //revProcessed = new HashSet<Integer>();
            
            newForwardCost =  (ArrayList<Long>[]) new ArrayList[n];
           // newReverseCost =  (ArrayList<Long>[]) new ArrayList[n]; 
            for (int i = 0; i < n; i++) {
                newForwardCost[i] = new ArrayList<Long>();
               // newReverseCost[i] = new ArrayList<Long>();
            ...
            //revqueue = new PriorityQueue<Entry>(n);
        }
        ...
        Long getEucldistance(int s, int t) {
            if (forwardEucldist[s] == INFINITY)
               ...
               // reverseEucldist[i] = (long) Math.sqrt((x[i] - x[s]) * (x[i] - x[s]) + (y[i] - y[s]) * (y[i] - y[s]));
            return forwardEucldist[s];
        }
        
        /**void computeForwardPotential() {
            for (int i = 0; i < n; i++) {
                forwardPotential[i]  = (forwardEucldist[i] - reverseEucldist[i]) / 2;
            }
        }
        */
        
        /**void computeNewEdgeWeightsForAstar() {
             ArrayList<Integer>[] frwGraph = adj[0];
             ArrayList<Integer>[] revGraph = adj[1];
             ArrayList<Integer>[] frwCost = cost[0];
             ArrayList<Integer>[] revCost = cost[1];
             
             for (int i = 0; i < n; i++) {
                  for (int j = 0; j < frwCost[i].size(); j++) {
                      long newFrwWeight = (frwCost[i].get(j) - forwardEucldist[i] + forwardEucldist[frwGraph[i].get(j)]);
                      newForwardCost[i].add(newFrwWeight);
                  }
                  
                  for (int j = 0; j < revCost[i].size(); j++) {
                      long newRevWeight = revCost[i].get(j) + forwardPotential[i] - forwardPotential[revGraph[i].get(j)];
                      newReverseCost[i].add(newRevWeight);
                  }
                  
             }
        }
         * @param getEucldistance 
        */
        ...
        long query(int s, int t) {
            ...
            ArrayList<Integer>[] frwCost = cost[0];
            //ArrayList<Integer>[] reverseGraph = adj[1];
            //computeEucldistances(s, t);
            //computeForwardPotential();
           // computeNewEdgeWeightsForAstar();
            frwdistance[s] = 0L;
            //revdistance[t] = 0L;
            ...
            //return findActualDistanceFromProcessedNodes(s, t);
        }

        
       /** long findActualDistanceFromProcessedNodes (int s, int t) {
            ...
            if(minDistance >= INFINITY)
                return -1L;
            return minDistance + forwardPotential[s] - forwardPotential[t];
        }
        */
